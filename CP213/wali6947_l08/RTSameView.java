...
public class RTSameView extends JPanel {
	...
	private class ButtonListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent evt) {
			...
			if (direction == 1) {
				height++;
				base++;
				RTSameView.this.model.setBase(base);
				RTSameView.this.model.setHeight(height);
			}
			if (direction == -1) {
				height--;
				base--;
	...
	private class ValuesListener implements PropertyChangeListener {
		...
		public void propertyChange(final PropertyChangeEvent evt) {
			...
			double base = model.getBase();
			double height = model.getHeight();
			//if base is changed, we see if it went up or down. 
			if(base == height) {
			...
			RTSameView.this.hypo.setText(DECIMAL_FORMAT.format(hypotenuse));
			//we create variables we can work with, and then use the setText methods and the decimal format to change them
		}
	}
