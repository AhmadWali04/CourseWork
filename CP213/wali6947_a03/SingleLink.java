...
public abstract class SingleLink<T> implements Iterable<T> {
	...
	SingleNode<T> sourceHead = source.front;
	source.front = source.front.getNext();
	if(source.front == null) {
		source.rear = null;
	}
	sourceHead.setNext(this.front);
	if(this.rear == null) {
		this.rear = sourceHead;
	}
	this.front =sourceHead;
	source.length--;
	this.length++;
	return;
	...
	final SingleNode<T> sourceHead = source.front;
	source.front = source.front.getNext();
	if(source.front ==null) {
		source.rear = null;
	}else {
		this.rear.setNext(sourceHead);
	}
	this.rear = sourceHead;
	source.length--;
	this.length++;
