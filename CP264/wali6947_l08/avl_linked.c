...
static avl_node* avl_rotate_left(avl_node *node) {
    //make the right node my new pivot, and then reassign their 
...
/**
 * Inserts item into an AVL tree. Insertion must preserve the AVL definition.
 * Only one of item may be in source.
 *
 * @param source - pointer to the AVL tree.
...
static bool avl_insert_aux(avl_linked *source, avl_node **node, const data_ptr item) {
    // Base case: Insert a new node when reaching a NULL position
    if (*node == NULL) {
        *node = avl_node_initialize(item);  // Allocate and initialize the new node
        if (*node == NULL) return false;  // Ensure memory allocation was successful
        source->count++;  // Increase the count of nodes in the AVL tree
        return true;  // Successfully inserted
    }

    int comp = data_compare(item, (*node)->item);  // Compare the item with the current node

    bool inserted = false;  // Track if insertion happened
    ...
    if (comp < 0) {
        ...
        // Item is greater, go to the right subtree
        avl_rebalance(node);
    ...
    // If insertion happened, update height and rebalance the tree
    if (inserted) {
        avl_update_height(*node);  
        avl_rebalance(node);       
    }
