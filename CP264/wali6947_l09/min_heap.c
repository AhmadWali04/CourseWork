...
//for a heap, the root node must be the smallest amongst all of its descendents
//a node must have complete kids in it
...
static void heapify_down(min_heap *source) {
    ...
    while (index < source->count) {
        ...
        if (right < source->count && source->values[right] < source->values[smallest]) {
            smallest = right;
        }
