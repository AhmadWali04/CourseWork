...
/*  auxilariy function to free bst*/
...
static void bst_inorder_aux(bst_node *node, data_ptr *items, int *index) {
    if (node != NULL) {
...
bool bst_max(const bst_linked *source, data_ptr item) {
    while(data_compare(source->root->item,item)==-1 && source!=NULL){
